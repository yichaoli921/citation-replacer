...
4. 把风格草稿整理成 `tmp_<期刊>文献引用风格.md`
5. 按这个 Markdown 风格备忘处理正文引用和参考文献
6. 如果这个期刊以后会反复使用，再把规则沉淀到 `references/journal-styles/<name>.md`

这个 skill 的默认逻辑是“样文驱动”，不是“只有内置期刊才能用”。

---

## Q8: DOI 我应该写还是不写？

**A**：先看本轮两篇目标期刊样文。若样文没有稳定保留 DOI，投稿版默认不写 DOI，除非：
...
旧的 `journal-styles/` 文件只能作参考，不能替代目标期刊最新样文。

---

## Q9: 编号是按作者拼音排还是正文首次出现？

**A**：先看目标期刊样文。如果样文是顺序编码制，就按正文首次出现顺序编号。

大多数中文编号制期刊不是按作者拼音，也不是按发表年份；但最终仍以本轮样文为准。
...
- ✅ 跨 run 替换逻辑
- ⚠️ 需要先用目标 SSci 期刊最新样文学习格式
- ⚠ 没有 APA / MLA / IEEE 等英文格式

如果你需要投 SSci，建议：

1. 用 `extract_style_from_samples.py` 从目标期刊学习风格
2. 把样文规则整理成 `tmp_<期刊>文献引用风格.md`
3. 再决定是否适合用本 skill 的 docx 替换脚本
